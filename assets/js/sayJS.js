      ...
      this.elements.forEach((form) => {
        ...
        inputs.forEach((el) => {
          ...
          if (el.hasAttribute("required") && value === "") {
            ...
            this.showError(
              el,
              `* ${el.placeholder || el.name || el.id} tidak boleh kosong!`
            );
          }
          if (
            el.type === "email" &&
            ...
            isValid = false;
            this.showError(el, "* alamat email tidak sesuai!");
          }
          if (
            el.hasAttribute("minlength") &&
            value.length < el.getAttribute("minlength")
          ) {
            isValid = false;
            this.showError(
              el,
              `* minimum adalah ${el.getAttribute("minlength")}`
            );
          }
          if (
            el.hasAttribute("maxlength") &&
            value.length > el.getAttribute("maxlength")
          ) {
            isValid = false;
            this.showError(
              el,
              `* maksimum adalah ${el.getAttribute("maxlength")}`
            );
          }
          if (el.type === "number") {
            ...
            if (isNaN(numberValue)) {
              isValid = false;
              this.showError(el, "* tidak boleh kosong! harus angka!");
            } else {
              if (min !== null && numberValue < min) {
                isValid = false;
                this.showError(
                  el,
                  `* harus lebih besar atau sama dengan ${min}`
                );
              }
              if (max !== null && numberValue > max) {
                ...
                this.showError(
                  el,
                  `* harus kurang dari atau sama dengan ${max}`
                );
              }
            }
          }
...
function editItem(event) {
  ...
  if (closest) {
    ...
    const baseURL =
      fullURL.substring(0, fullURL.lastIndexOf("/") + 1) + currentUrl;
...
function deleteItem(event, msg = "") {
  ...
  if (closest) {
    sayAlert(
      "confirmModal",
      "Confirm!",
      "Apakah yakin menghapus data ini?" + msg,
      "danger",
      true,
      () => {
        showLoading();
        ...
        fetch(url, {
          ...
          .catch((error) => {
            sayAlert(
              "errorModal",
              "Error",
              "Terjadi kesalahan pada sistem." + error.message,
              "warning"
            );
          })
          .finally(() => {
...
function formatCurrency(input, useDecimal) {
  ...
  if (value.charAt(0) === "0") value = value.slice(1); // Hapus '0' pertama
  if (value) {
    if (useDecimal) {
                  ...
                  <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="sayalert d-flex align-items-stretch">
                            ...
                            ${
                              isConfirm
                                ? `<button type="button" class="btn btn-sm btn-${type} ms-4 me-2" id="${modalId}ConfirmButton">${confirmButtonText}</button>
