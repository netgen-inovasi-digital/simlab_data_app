...
{
    private $table = 'penempatan_categories';
    private $id = 'id_penempatan';
    ...
    public function index()
    {
        ...
        return view('Modules\Penempatan\Views\v_penempatan', $data);
    }
    ...
    public function delete()
    {
        ...
        if ($id) {
            ...
            // Cek apakah penempatan masih digunakan oleh personel
            $cekPersonelPenempatan = $modelPersonel->where('id_penempatan', $id)->countAllResults();

            if ($cekPersonelPenempatan > 0) {
    ...
    public function submit()
    {
        ...
        if ($nama != "") {
            ...
            if ($existing > 0) {
                return $this->response->setJSON([
                    'res' => 'duplicate',
                    'message' => 'Penempatan dengan nama yang sama sudah ada.',
                    'xname' => csrf_token(),
                    'xhash' => csrf_hash()
                ]);
            }
        ...
        $model = new MyModel($this->table);
        if ($id == "") {
            $res = $model->insertData($data, true); // akan return id_penempatan
            $id = (string)$res;
        } else {
    ...
    public function dataList()
    {
        ...
        foreach ($list as $row) {
            $id = bin2hex($this->encrypter->encrypt($row->id_penempatan));
            $response = array();
            $response[] = $row->nama;
    ...
    function aksi($id)
    {
        return '<div id="' . $id . '" class="float-end">
			...
			<span class="text-danger btn-action" title="Hapus" onclick="deleteItemPenempatan(event)">
