...
<script>
    table = createTable({
        apiUrl: '<?php echo site_url("penempatan/datalist") ?>',
    });
    addAction();
    ...
    function deleteItemPenempatan(event, msg = "") {
        const closest = event.target.closest('div');
        if (msg != "") msg = '<br><strong>' + msg + '</strong>';
        if (closest) {
            const id = closest.getAttribute('id');
            sayAlert('confirmModal', 'Hapus Penempatan', 'Yakin ingin menghapus penempatan ini?', 'danger', true, () => {
                deleteData({
                    url: '<?= base_url("penempatan/delete") ?>',
                    data: {
                        id
                    },
                    onSuccess: () => {
                        loadContent('penempatan');
                        sayAlert('successModal', 'Success', 'Data berhasil dihapus.', 'success');
                    },
                    onError: () => {
                        sayAlert('errorModal', 'Error', 'Penempatan tidak bisa dihapus karena masih digunakan oleh personel yang ada.', 'warning');
                    }
                });
            });
        }
...
<div class="modal fade" id="modalForm" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog" role="document" style="margin: 2% auto">
        <div class="modal-content">
            ...
            <?php echo form_open('penempatan/submit', array('id' => 'myform', 'novalidate' => '')) ?>
            <div class="modal-body">
                <input type="hidden" value="" name="id" />
                <div class="row mb-2">
                    <label class="col-md-4 col-form-label">Nama Penempatan</label>
                    <div class="col">
