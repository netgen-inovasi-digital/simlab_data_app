...
<script>
  ...
  function addDragEvents(item) {
    ...
    item.addEventListener("dragstart", (e) => {
      ...
      dragStartTimeout = setTimeout(() => {
        ...
        adjustZoomViewportHeight(); // [FIX] Sesuaikan tinggi viewport setelah menyembunyikan item
      }, 0);
    });
    ...
    item.addEventListener("dragend", (e) => {
      ...
      try {
        ...
        adjustZoomViewportHeight(); // [FIX] Selalu sesuaikan tinggi di akhir
        if (draggedItem) {
  ...
  folderMenu.addEventListener("click", function(e) {
    ...
    // [FIX] Panggil penyesuaian tinggi setelah toggle selesai.
    // Diberi sedikit timeout untuk memastikan DOM selesai di-render.
    setTimeout(adjustZoomViewportHeight, 50);
  });
  ...
  /**
   * [BARU] Fungsi untuk menyesuaikan tinggi viewport zoom.
   * Ini dipisahkan agar bisa dipanggil dari berbagai tempat.
   */
  function adjustZoomViewportHeight() {
    const zoomViewport = document.getElementById('zoom-viewport');
    const folderContainer = document.getElementById('folder');
    const scaleValue = parseFloat(document.getElementById('zoom-slider')?.value || 1);
    if (!zoomViewport || !folderContainer) return;
    zoomViewport.style.height = `${folderContainer.scrollHeight * scaleValue}px`;
  }
