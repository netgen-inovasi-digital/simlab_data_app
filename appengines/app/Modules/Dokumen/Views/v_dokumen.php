    ...
    <div class="card">
      ...
      <div class="card-body ps-4 pe-4">
        <div id="dokumen">
          ...
          ?>
            <div id="<?= $id ?>" class="dokumen-item <?= ($row->kode_induk != 0) ? 'child' : '' ?>" draggable="true">
...
<script>
  ...
  function addDragEvents(item) {
    item.addEventListener("dragstart", (e) => {
      ...
      setTimeout(() => {
        dokumen.insertBefore(placeholder, item.nextSibling);
    ...
    item.addEventListener("dragend", (e) => {
      ...
      const currentIndex = [...dokumen.children].indexOf(placeholder);
      const previousItem = dokumen.children[currentIndex - 1];

      if (e.clientX - dragStartX > 20 && dokumen.firstElementChild !== draggedItem) {
        let potentialParent = previousItem;
        while (potentialParent && potentialParent.classList.contains("child")) {
          const idx = [...dokumen.children].indexOf(potentialParent);
          potentialParent = dokumen.children[idx - 1];
        }
        if (potentialParent) {
      ...
      dokumen.insertBefore(draggedItem, placeholder);
      placeholder.remove();
      updateKodeDokumen();
      saveAll();
    });
    ...
    item.addEventListener("dragover", (e) => {
      e.preventDefault();
      const after = getDragAfterElement(dokumen, e.clientY);
      if (after == null)
        dokumen.appendChild(placeholder);
      else dokumen.insertBefore(placeholder, after);
    });
  }

  function getDragAfterElement(container, y) {
    const elements = [...container.querySelectorAll(".dokumen-item:not([style*='display: none'])")];
  ...
  function updateKodeDokumen() {
    const items = [...document.querySelectorAll("#dokumen .dokumen-item")];
    let mainCounter = 0;
    let childCounter = 0;
    let currentMainKode = '';
    let maxCode = 0;
  ...
  function saveAll() {
    ...
    const items = document.querySelectorAll(".dokumen-item");
  ...
  <div class="modal-dialog" role="document" style="margin: 2% auto">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="row mb-2">
          <label class="col-md-4 col-form-label">Nama Dokumen</label>
          <div class="col">
